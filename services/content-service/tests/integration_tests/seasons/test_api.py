...
async def test_get_seasons(ac, get_all_series, get_all_seasons, page, per_page):
    for series in get_all_series:
        ...
        data = await get_and_validate(
            ac, "/seasons", params={"series_id": series["id"], "page": page, "per_page": per_page}
        )
...
async def test_add_season_valid(ac, get_all_series, title, season_number):
    for i in range(2):
        ...
        season = await get_and_validate(ac, f"/seasons/{season_id}", expect_list=False)
...
async def test_add_season_invalid(ac, get_all_series, field, value):
    ...
    res = await ac.post("/seasons", json=req_body)
...
async def test_update_season_title_valid(ac, get_all_seasons, title):
    for i in range(2):
        ...
        res = await ac.patch(f"/seasons/{season_id}", json={"title": title})
...
async def test_update_season_number_valid(ac, get_all_series, get_all_seasons, season_number):
    for i in range(2):
        ...
        seasons = await get_and_validate(ac, "/seasons", params={"series_id": series_id})
        season_id = seasons[0]["id"]

        res = await ac.patch(f"/seasons/{season_id}", json={"season_number": season_number})
        assert res.status_code == 200
...
async def test_update_season_invalid(ac, get_all_seasons, field, value):
    ...
    res = await ac.patch(f"/seasons/{season_id}", json={field: value})
    assert res.status_code == 422
...
async def test_update_season_on_conflict(ac, get_all_series):
    ...
    res = await ac.patch(f"/seasons/{seasons[0]['id']}", json={"season_number": 20})
    assert res.status_code == 200

    res = await ac.patch(f"/seasons/{seasons[1]['id']}", json={"season_number": 20})
    assert res.status_code == 409


async def test_update_season_not_found(ac):
    for _ in range(2):
        res = await ac.patch(f"/seasons/{str(uuid4())}", json={"title": "Valid Season Title 7"})
        assert res.status_code == 404


async def test_delete_season(ac, get_all_seasons):
    for season in get_all_seasons[4:]:
        assert (await ac.get(f"/seasons/{season['id']}")).status_code == 200
        assert (await ac.delete(f"/seasons/{season['id']}")).status_code == 204
        assert (await ac.get(f"/seasons/{season['id']}")).status_code == 404
