...
async def test_pagination(ac, page, per_page, get_all_series):
    data = await get_and_validate(ac, "/v1/series", params={"page": page, "per_page": per_page})
...
async def test_series_sorting(ac, field, order, max_pagination):
    data = await get_and_validate(
        ac, "/v1/series", params={"sort": f"{field}:{order}", **max_pagination}
    )
...
async def test_filter_by_title(ac, title, get_all_series, max_pagination):
    data = await get_and_validate(ac, "/v1/series", params={"title": title, **max_pagination})
...
async def test_filter_by_director(ac, director, get_all_series, max_pagination):
    data = await get_and_validate(ac, "/v1/series", params={"director": director, **max_pagination})
...
async def test_filter_by_description(ac, description, get_all_series, max_pagination):
    data = await get_and_validate(
        ac, "/v1/series", params={"description": description, **max_pagination}
    )
...
async def test_filter_by_release_year(ac, year, get_all_series, max_pagination):
    ...
    data = await get_and_validate(ac, "/v1/series", params={"year": year, **max_pagination})
...
async def test_filter_by_release_year_gt(ac, year_gt, get_all_series, max_pagination):
    ...
    data = await get_and_validate(ac, "/v1/series", params={"year_gt": year_gt, **max_pagination})
    assert len(data) == expected_count
...
async def test_filter_by_release_year_lt(ac, year_lt, get_all_series, max_pagination):
    ...
    data = await get_and_validate(ac, "/v1/series", params={"year_lt": year_lt, **max_pagination})
    assert len(data) == expected_count
...
async def test_filter_by_release_year_range(ac, year_gt, year_lt, get_all_series, max_pagination):
    ...
    data = await get_and_validate(
        ac, "/v1/series", params={"year_gt": year_gt, "year_lt": year_lt, **max_pagination}
    )
    assert len(data) == expected_count
...
async def test_filter_by_rating(ac, rating, get_all_series, max_pagination):
    ...
    data = await get_and_validate(ac, "/v1/series", params={"rating": rating, **max_pagination})
    assert len(data) == expected_count
...
async def test_filter_by_rating_gt(ac, rating_gt, get_all_series, max_pagination):
    ...
    data = await get_and_validate(ac, "/v1/series", params={"rating_gt": rating_gt, **max_pagination})
    assert len(data) == expected_count
...
async def test_filter_by_rating_lt(ac, rating_lt, get_all_series, max_pagination):
    ...
    data = await get_and_validate(ac, "/v1/series", params={"rating_lt": rating_lt, **max_pagination})
    assert len(data) == expected_count
...
async def test_filter_by_rating_range(ac, rating_gt, rating_lt, get_all_series, max_pagination):
    ...
    data = await get_and_validate(
        ac, "/v1/series", params={"rating_gt": rating_gt, "rating_lt": rating_lt, **max_pagination}
    )
    assert len(data) == expected_count
...
async def test_filter_by_genres(ac, genres_ids, get_all_series_with_rels, max_pagination):
    ...
    data = await get_and_validate(
        ac, "/v1/series", params={"genres_ids": genres_ids, **max_pagination}
    )
    assert len(data) == expected_count
...
async def test_get_filter_by_actors(
    ...
    data = await get_and_validate(
        ac, "/v1/series", params={"actors_ids": actors_ids, **max_pagination}
    )
    assert len(data) == expected_count
...
async def test_add_series_without_optional_valid(ac, title, description, director, release_year):
    ...
    series = await get_and_validate(ac=ac, url=f"/v1/series/{series_id}", expect_list=False)
...
async def test_add_series_invalid(ac, field, invalid_value):
    ...
    res = await ac.post("/v1/series", json=valid_data)
...
async def test_series_on_conflict(ac):
    ...
    res = await ac.post("/v1/series", json=req_body)
    assert res.status_code == 201
...
async def test_update_field_valid(ac, get_all_series, field, value):
    ...
    res = await ac.patch(f"/v1/series/{series_id}", json={field: value})
    assert res.status_code == 200

    data = await get_and_validate(ac, f"/v1/series/{series_id}", expect_list=False)
...
async def test_update_series_genres(ac, get_all_series, genres_ids):
    ...
    res = await ac.patch(f"/v1/series/{series_id}", json={"genres_ids": genres_ids})
    assert res.status_code == 200
...
async def test_update_series_actors(ac, get_all_actors, get_all_series, actors_indexes):
    ...
    res = await ac.patch(f"/v1/series/{series_id}", json={"actors_ids": actors_ids})
    assert res.status_code == 200
...
async def test_delete_series(ac, created_series):
    for series in created_series:
        assert (await ac.get(f"/v1/series/{series['id']}")).status_code == 200
        assert (await ac.delete(f"/v1/series/{series['id']}")).status_code == 204
        assert (await ac.get(f"/v1/series/{series['id']}")).status_code == 404
