...
async def test_movies_sorting(ac, field, order, max_pagination):
    data = await get_and_validate(
        ac, "/v1/movies", params={"sort": f"{field}:{order}", **max_pagination}
    )
...
async def test_filter_by_title(ac, title, get_all_movies, max_pagination):
    data = await get_and_validate(
        ac=ac, url="/v1/movies", params={"title": title, **max_pagination}
    )
    assert len(data) == count_content(items=get_all_movies, field="title", value=title)
...
async def test_filter_by_description(ac, description, get_all_movies, max_pagination):
    data = await get_and_validate(
        ac=ac, url="/v1/movies", params={"description": description, **max_pagination}
    )
    assert len(data) == count_content(items=get_all_movies, field="description", value=description)
...
async def test_filter_by_exact_rating(ac, rating, get_all_movies, max_pagination):
    expected_count = sum(1 for movie in get_all_movies if float(rating) == float(movie["rating"]))
    data = await get_and_validate(
        ac=ac, url="/v1/movies", params={"rating": rating, **max_pagination}
    )
    assert len(data) == expected_count
...
async def test_filter_by_rating_gt(ac, rating_gt, get_all_movies, max_pagination):
    expected_count = sum(1 for movie in get_all_movies if float(rating_gt) < float(movie["rating"]))
    data = await get_and_validate(
        ac=ac, url="/v1/movies", params={"rating_gt": rating_gt, **max_pagination}
    )
    assert len(data) == expected_count
...
async def test_filter_by_rating_lt(ac, rating_lt, get_all_movies, max_pagination):
    expected_count = sum(1 for movie in get_all_movies if float(rating_lt) > float(movie["rating"]))
    data = await get_and_validate(
        ac=ac, url="/v1/movies", params={"rating_lt": rating_lt, **max_pagination}
    )
    assert len(data) == expected_count
...
async def test_get_filter_by_genres(ac, genres_ids, get_all_movies_with_rels, max_pagination):
    ...
    data = await get_and_validate(
        ac=ac, url="/v1/movies", params={"genres_ids": genres_ids, **max_pagination}
    )
    assert len(data) == expected_count
...
async def test_add_valid_data_without_optional(ac, title, description, release_year, duration):
    ...
    movie = await get_and_validate(ac=ac, url=f"/v1/movies/{movie_id}", expect_list=False)
...
async def test_update_field_valid(ac, get_all_movies, field, value):
    ...
    data = await get_and_validate(ac, f"/v1/movies/{movie_id}", expect_list=False)
...
async def test_update_field_invalid(ac, get_all_movies, field, value):
    movie_id = get_all_movies[0]["id"]
    res = await ac.patch(f"/v1/movies/{movie_id}", json={field: value})
    assert res.status_code == 422
    assert "detail" in res.json()
